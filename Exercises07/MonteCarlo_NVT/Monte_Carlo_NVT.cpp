...
{ 
  ...
  // ofstream eq_potenergy("equilibration_v.out");
  // ofstream eq_virial("equilibration_w.out");

  for (int i = 0; i < 4000; ++i){
  	Move();
  	// Measure();
  	// eq_potenergy << walker[iv] + vtail*(double)npart << endl;
  	// eq_virial << rho * temp + (walker[iw] + (double)npart * ptail) / vol << endl;
  }
  ...
  /*
  ofstream pressure, potenergy;

  potenergy.open("v_instant_values.out");
  pressure.open("p_instant_values.out");
  ...
  for (int i = 0; i < 500000; ++i){
  	...
  	Move();
  	Measure();
  	potenergy << walker[iv]/(double)npart  << endl;
  	pressure << rho * temp + (walker[iw] + (double)npart * ptail) / vol  << endl;
  }
...
//update of the histogram of g(r)
     if (dr < box/2.)
     	walker[igofr + (int) floor((double)nbins * (2. * dr / box))] += 2.;
...
//averages of g(r)
    for (int i = 0; i < nbins; ++i){
    	...
    	gdir = blk_av[igofr + i] / blk_norm / (rho * (double)npart * 4. * M_PI * dr3 / 3.);
    	glob_av[igofr + i] += gdir;
    	glob_av2[igofr + i] += gdir*gdir;
    	err_gofr=Error(glob_av[igofr +i],glob_av2[igofr + i],iblk);
    	Gofr << scientific << gdir << " ";
    	Gave << scientific <<  ((double)i+0.5)*bin_size << " " << glob_av[igofr + i]/(double)iblk << " " << err_gofr << endl;
    }
    Gofr << endl;
