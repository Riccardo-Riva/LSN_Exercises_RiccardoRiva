...
{
	//return 0;
	//return x*xxexp(-0.5*x*x) - exp(-0.5*x*x);
	
	double mu = 0.80;
	double sigma = 0.62;
	double psi = exp( -pow(x-mu,2) / (2.*pow(sigma,2)) ) + exp( -pow(x+mu,2) / (2.*pow(sigma,2)));
	return psi*((pow(x,2)+pow(mu,2))/pow(sigma,2)-1)/pow(sigma,2) + 2*x*mu*(-exp( -pow(x-mu,2) / (2.*pow(sigma,2)) ) + exp( -pow(x+mu,2) / (2.*pow(sigma,2))))/pow(sigma,4);
